...
bool script_checker::verify_ecdsa_signature(
    ...
    uint32_t input_sequence) const NOEXCEPT
{
    ...
    if (transaction_version < chain::relative_locktime_min_version)
        return error::op_check_sequence_verify2;

    // The transaction sequence is disabled.
    if (get_right(input_sequence, chain::relative_locktime_disabled_bit))
        return error::op_check_sequence_verify3;

    // The stack sequence type differs from that of tx input.
    if (get_right(stack_sequence, chain::relative_locktime_time_locked_bit) !=
        get_right(input_sequence, chain::relative_locktime_time_locked_bit))
        return error::op_check_sequence_verify4;

    // The unmasked stack sequence is greater than that of tx sequence.
    if (mask_left(stack_sequence, chain::relative_locktime_mask_left) >
        mask_left(input_sequence, chain::relative_locktime_mask_left))
